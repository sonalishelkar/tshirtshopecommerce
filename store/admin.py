...
from store.models import Tshirt,Payment, Order, OrderItem, Cart, Brand, Color, IdealFor, NeckType, Occasion, Sleeve, SizeVariant, tshirt
...
class OrderConfiguration(admin.ModelAdmin):
    list_display =  ['user', 'shipping_address', 'phone', 'date', 'orderStatus']
    readonly_fields = ('user', 'phone', 'shipping_address', 'total', 'payment_method', 
                        'payment', 'payment_request_id', 'payment_id', 'payment_status')
    fieldsets = (
        ("Order Information", {"fields" : ('orderStatus', 'shipping_address', 'phone', 'total', 'user')}),
